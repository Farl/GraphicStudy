...
{
	[Serializable]
	[NodeAttributes( "Custom Texture Coordinates", "UV Coordinates", "Texture UV coordinates set, if <b>Tex</b> is connected to a texture object it will use that texture scale factors, otherwise uses <b>Tilling</b> and <b>Offset</b> port values", null )]
	public sealed class CustomTextureCoordinatesNode : ParentNode
	{
		...
		public override void DrawProperties()
		{
			...
			m_overrideID = EditorGUILayout.TextField ("Override ID", m_overrideID);
		...
		public override string GenerateShaderForOutput( int outputId, ref MasterNodeDataCollector dataCollector, bool ignoreLocalVar )
		{
			...
			if( dataCollector.MasterNodeCategory == AvailableShaderTypes.Template )
			{
				...
				if( !string.IsNullOrEmpty( currPropertyName ) || !string.IsNullOrEmpty(m_overrideID) )
				{
					// Farl: override current property name
					if (!string.IsNullOrEmpty (m_overrideID)) {
						currPropertyName = m_overrideID;
					}
