...
public class CreateShaders : EditorWindow {
    ...
    private static string createPath = null;
    private static string defaultCreatePath = "TestResources/{0}/Game";
    ...
    void CheckCreatePath()
    {
        ...
        if (!createPath.Contains("{0}"))
        {
            createPath = createPath + "{0}";
        }
    }
	...
	void OnEnable()
	{
        ...
        createPath = EditorPrefs.GetString("CreateShader.createPath", defaultCreatePath);
	...
	void UpdateProperty()
	{
        ...
        EditorPrefs.SetString("CreateShader.createPath", createPath);
        CheckCreatePath();
		...
		if (useShaderOwnFolder) {
			...
			shaderRelativeFolder = string.Format(createPath + "/{1}", shaderUserName, shaderName);
    ...
    private void OnGUI()
    {
		...
		EditorGUI.BeginChangeCheck ();
		{
            ...
            EditorGUI.BeginChangeCheck();
            createPath = EditorGUILayout.DelayedTextField("Create Path", createPath);
